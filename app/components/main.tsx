...
const meals = [
  {
    meal_NM: "석식",
    meal: ["쌀밥", "돈육김치찌개 (5.9.10)", "볼어묵야채볶음 (1.5.6)", "오이무침 (5.6.13)", "치즈달걀말이 (1.2.5)", "소품떡/소스 (2.5.6.10.12.13.15.16)", "돌산갓김치 (9)", "짜먹는요구르트(딸기) (2)"]
  },
  {
    meal_NM: "조식",
    meal: ["쌀밥", "돈육김치찌개 (5.9.10)", "볼어묵야채볶음 (1.5.6)", "오이무침 (5.6.13)", "치즈달걀말이 (1.2.5)", "소품떡/소스 (2.5.6.10.12.13.15.16)", "돌산갓김치 (9)", "짜먹는요구르트(딸기) (2)"]
        ...
        <div className="flex-grow md:w-7/10 h-full">
          <Card className="h-full gap-0">
            <CardHeader className="pb-2">
              <CardTitle className="text-2xl pb-1">분실물 안내 (LNF)</CardTitle>
            </CardHeader>
        ...
        <div className="md:w-3/10">
          <Card className="h-full gap-0">
            <CardHeader className="pb-0">
              <CardTitle className="text-2xl pb-1">오늘의 식단</CardTitle>
            </CardHeader>
            <CardContent>
              {meals.map((meal, index) => (
                <>
                  <h3 className={`mb-2 text-base font-bold${index != 0 ? " pt-2" : ""}`} key={index}>{(meals[0].meal_NM == "석식" && index == 1) ? `다음날 ${meal.meal_NM}` : meal.meal_NM}</h3>
                  <Card className="pb-2.5 pt-2.5">
                    <CardContent>
